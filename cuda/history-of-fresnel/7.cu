...
/*計算に必要な変数の定義*/
	...
	for(k=0; k<*points_d; k++){
		lumi_intensity_d[i*width+j]=lumi_intensity_d[i*width+j]+cosf(wave_num*((j-x_d[k])*(j-x_d[k])+(i-y_d[k])*(i-y_d[k]))*z_d[k]);
	}
}
...
/*デバイスのメモリ解放*/
    ...
    //最大・最小値用の変数を比較できるようにとりあえずlumi~[0]を入れる
