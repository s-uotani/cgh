...
/*CGH作成の式を用いて計算*/
	...
	for(i=0; i<heigth; i++){
		for(j=0; j<width; j++){
			for(k=0; k<points; k++){
				lumi_intensity[i*width+j]=lumi_intensity[i*width+j]+cos(pixel_int*wave_num*sqrt((j-x[k])*(j-x[k])+(i-y[k])*(i-y[k])+z[k]*z[k]));
			}
		}
	}
