...
/*フレネル近似式の関数*/
double func_fresnel(int xj, int yj, double zi, int xa, int ya){
	double r_ja;
	int dx, dy;

	dx=xa-xj;
	dy=ya-yj;
	r_ja=(dx*dx+dy*dy)/zi;	//暗黙の型変換でdouble型
...
/*読み込むデータのアドレス，データのサイズ，データの個数，格納するファイルポインタを指定*/
...
/*各バッファに物体点座標を取り込み，ホログラム面と物体点の位置を考慮したデータを各配列に入れる*/
	for(i=0; i<points; i++){
		...
		x[i]=x_buf*40+width/2;	//物体点を離すために物体点座標に40を掛け，中心の座標を足す
		y[i]=y_buf*40+heigth/2;
		z[i]=((double)z_buf)*40+pow(10, 4);
	}
	fclose(fp);
...
/*計算に必要な変数の定義*/
	...
	double pixel_int=10.5;	//画素間隔
	double wave_len=0.633;	//光波長
	...
	for(i=0; i<width*heigth; i++){
		lumi_intensity[i]=0.0;	//光強度用の配列を0クリア
	}
...
/*フレネル近似式を用いて計算*/
	...
	for(i=0; i<heigth; i++){
		for(j=0; j<width; j++){
			for(k=0; k<points; k++){
				lumi_intensity[i*width+j]=lumi_intensity[i*width+j]+cos(wave_num*func_fresnel(x[k], y[k], z[k], j, i));
			}
		}
	}
