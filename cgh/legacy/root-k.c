...
/*cghの計算式の関数*/
double func_cgh(int xj, int yj, double zj, int xa, int ya){
	double r_ja;
	int dx, dy;

	dx=xa-xj;
	dy=ya-yj;
	r_ja=sqrt(dx*dx+dy*dy+zj*zj);	//暗黙の型変換でdouble型
...
/*CGH作成の式を用いて計算*/
	...
	for(i=0; i<heigth; i++){
		for(j=0; j<width; j++){
			for(k=0; k<points; k++){
				lumi_intensity[i*width+j]=lumi_intensity[i*width+j]+cos(pixel_int*wave_num*func_cgh(x[k], y[k], z[k], j, i));
			}
		}
	}
