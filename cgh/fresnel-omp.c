...
/*フレネル近似式を用いて計算*/
	...
	for(i=0; i<heigth; i++){
		for(j=0; j<width; j++){
			for(k=0; k<284; k++){
				lumi_intensity[i*width+j]=lumi_intensity[i*width+j]+cosf(wave_num*((j-x[k])*(j-x[k])+(i-y[k])*(i-y[k]))*z[k]);
			}
		}
	}
...
/*書き込むデータのアドレス，データのサイズ，データの個数，ファイルのポインタを指定*/
	...
	printf("'fresnel-omp.bmp' was saved.\n");
