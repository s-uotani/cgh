...
/*フレネル近似のカーネル関数*/
__global__ void fresnel_gpu(int *x_d, int *y_d, double *z_d, double *lumi_intensity_d, int *points_d){
    int i, j, k;
...
/*計算に必要な変数の定義*/
	...
	double wave_num=M_PI/wave_len; //波数の2分の1

	for(j=0; j<width; j++){
		for(k=0; k<*points_d; k++){
			lumi_intensity_d[i*width+j]=lumi_intensity_d[i*width+j]+cos(wave_num*((j-x_d[k])*(j-x_d[k])+(i-y_d[k])*(i-y_d[k]))/z_d[k]);
		}
	}
}
...
/*カーネル関数の起動*/
	fresnel_gpu<<< grid, block >>>(x_d, y_d, z_d, lumi_intensity_d, points_d);
