...
/*cghの計算式のカーネル関数*/
__global__ void func_cgh_gpu(int *x_d, int *y_d, double *z_d, double *lumi_intensity_d, int *points_d){
	int i, j, k;

	i=blockDim.x*blockIdx.x+threadIdx.x;	//widthのループの置き換え
//	j=blockDim.y*blockIdx.y+threadIdx.y;	//heigthのループの置き換え

	//計算に必要な変数の定義
	double interval=10.5F;				//画素間隔
	double wave_len=0.633F;				//光波長
	double wave_num=2.0*M_PI/wave_len;	//波数

	for(j=0; j<width; j++){
		for(k=0; k<*points_d; k++){
			lumi_intensity_d[i*width+j]=lumi_intensity_d[i*width+j]+cos(interval*wave_num*sqrt((j-x_d[k])*(j-x_d[k])+(i-y_d[k])*(i-y_d[k])+z_d[k]*z_d[k]));
		}
	}
}
...
/*ホスト側の変数*/
	int i, j;
	int points;					//物体点
	double *lumi_intensity;		//光強度用の配列
	double min, max, mid;		//2値化に用いる
...
/*各バッファに物体点座標を取り込み，ホログラム面と物体点の位置を考慮したデータを各配列に入れる*/
	for(i=0; i<points; i++){
		...
		z[i]=((double)z_buf)*40+100000.0F;
	}
	fclose(fp);

	lumi_intensity=(double *)malloc(sizeof(double)*width*heigth);	//malloc関数でメモリを動的に確保

/*デバイス側の変数*/
	int *x_d, *y_d;
	double *z_d;
	double *lumi_intensity_d;
	int *points_d;

	dim3 block(1024,1,1);  //ブロックサイズ(スレッド数)の配置
    dim3 grid(ceil(width/block.x),1,1);     //グリッドサイズ(ブロック数)の配置
...
/*デバイス側のメモリ確保*/
	...
	cudaMalloc((void**)&lumi_intensity_d, width*heigth*sizeof(double));
...
/*ホスト側からデバイス側へデータ転送*/
	...
	cudaMemcpy(lumi_intensity_d, lumi_intensity, width*heigth*sizeof(double), cudaMemcpyHostToDevice);
...
/*デバイス側からホスト側へデータ転送*/
	cudaMemcpy(lumi_intensity, lumi_intensity_d, width*heigth*sizeof(double), cudaMemcpyDeviceToHost);
