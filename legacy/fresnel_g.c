...
double lumi_intensity[width*heigth];	//光強度用の配列
...
/*各バッファに物体点座標を取り込み，ホログラム面と物体点の位置を考慮したデータを各配列に入れる*/
	for(i=0; i<points; i++){
		...
		x[i]=x_buf*40+0.5*width;	//物体点を離すために物体点座標に40を掛け，中心の座標を足す
		y[i]=y_buf*40+0.5*heigth;
		z[i]=((double)z_buf)*40+10000;
	}
	fclose(fp);
...
//	lumi_intensity=(double *)malloc(sizeof(double)*width*heigth);	//malloc関数でメモリを動的に確保
	for(i=0; i<width*heigth; i++){
		lumi_intensity[i]=0.0F;	//光強度用の配列を0クリア
	}
...
/*フレネル近似式を用いて計算*/
	...
	for(i=0; i<heigth; i++){
		for(j=0; j<width; j++){
			for(k=0; k<points; k++){
				lumi_intensity[i*width+j]=lumi_intensity[i*width+j]+cos(wave_num*((j-x[k])*(j-x[k])+(i-y[k])*(i-y[k]))/z[k]);
			}
		}
	}
